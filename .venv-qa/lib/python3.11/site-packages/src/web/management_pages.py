...
def _geo_label(offer: Offer) -> str:
    if offer.geo_scope == 'all_russia':
        return 'Вся Россия'
    parts = [value for value in (offer.city, offer.region) if value]
    if parts:
...
def offers_page(
    ...
    offer_type: str = Query(''), region: str = Query(''), city: str = Query(''), page: int = Query(1, ge=1),
):
    redirect = _require_setup()
    if redirect:
        return redirect
    page_size = 40
    q, status, category, offer_type, region, city = [value.strip() for value in (q, status, category, offer_type, region, city)]
    ...
    with create_session() as session:
        ...
        if q:
            token = f'%{q}%'
            filters_sql.append(or_(Offer.title.ilike(token), Offer.display_title.ilike(token), Offer.merchant.ilike(token), Offer.promo_code.ilike(token), Offer.city.ilike(token), Offer.region.ilike(token), Offer.conditions.ilike(token)))
        if status: filters_sql.append(Offer.status == status)
        if category: filters_sql.append(Offer.category == category)
        if offer_type: filters_sql.append(Offer.offer_type == offer_type)
        if region: filters_sql.append(Offer.region == region)
        if city: filters_sql.append(Offer.city == city)
        if filters_sql:
            query = query.where(*filters_sql)
            count_query = count_query.where(*filters_sql)
        total = int(session.scalar(count_query) or 0)
        offers = list(session.scalars(query.order_by(Offer.last_seen_at.desc()).offset((page - 1) * page_size).limit(page_size)).all())
        categories = [v for v in session.scalars(select(Offer.category).where(Offer.category.is_not(None), Offer.category != '').distinct().order_by(Offer.category)).all() if v]
        regions = [v for v in session.scalars(select(Offer.region).where(Offer.region.is_not(None), Offer.region != '').distinct().order_by(Offer.region)).all() if v]
        cities = [v for v in session.scalars(select(Offer.city).where(Offer.city.is_not(None), Offer.city != '').distinct().order_by(Offer.city)).all() if v]
    ...
    filter_html = f'''<div class="card"><form method="get" class="filters">
      <div class="field"><label>Поиск</label><input name="q" value="{html.escape(q)}" placeholder="Название, магазин, промокод, ГЕО, условия"></div>
    ...
    for offer in offers:
        ...
        condition_text = offer.conditions or '—'
        rows.append(f'''<tr><td><a href="/offers/{offer.id}">#{offer.id}</a></td>
          <td><b>{html.escape(offer.display_title or offer.title)}</b><div class="muted">{html.escape(offer.merchant or '—')}</div></td>
          <td>{html.escape(_geo_label(offer))}</td><td class="conditions">{html.escape(condition_text[:320])}</td>
          <td><span class="pill">{html.escape(offer.status)}</span></td><td>{html.escape(offer.category or '—')}<div class="muted">{html.escape(offer.subcategory or '')}</div></td>
          <td>{html.escape(offer.offer_type)}</td><td>{html.escape(str(benefit))}</td><td>{html.escape(str(offer.last_seen_at)[:19])}</td></tr>''')
    table = '<p class="muted">Ничего не найдено.</p>' if not rows else f'''<table class="table"><thead><tr><th>ID</th><th>Предложение</th><th>ГЕО</th><th>Условия</th><th>Статус</th><th>Категория</th><th>Тип</th><th>Выгода</th><th>Последний раз</th></tr></thead><tbody>{''.join(rows)}</tbody></table>'''
    pages = max(1, math.ceil(total / page_size))
    base = {'q': q, 'status': status, 'category': category, 'offer_type': offer_type, 'region': region, 'city': city}
    pagination = '<div class="pagination">'
    if page > 1: pagination += f'<a href="/offers?{urlencode({**base, "page": page-1})}">← Назад</a>'
    pagination += f'<span class="muted">Страница {page} из {pages} · всего {total}</span>'
    if page < pages: pagination += f'<a href="/offers?{urlencode({**base, "page": page+1})}">Далее →</a>'
    pagination += '</div>'
    return _layout('Предложения', filter_html + f'<div class="card">{table}{pagination}</div>', 'offers')
...
def offer_detail(offer_id: int):
    ...
    with create_session() as session:
        ...
        if offer is None: return _layout('Предложение не найдено', '<div class="card">Такого Offer нет.</div>', 'offers')
        observations = session.execute(select(OfferSourceObservation, Source).join(Source, Source.id == OfferSourceObservation.source_id).where(OfferSourceObservation.offer_id == offer_id).order_by(OfferSourceObservation.observed_at.desc())).all()
    ...
    max_discount = f'{offer.max_discount_amount:g} ₽' if offer.max_discount_amount is not None else '—'
    min_order = f'{offer.min_order_amount:g} ₽' if offer.min_order_amount is not None else '—'
    kv = [
        ('Статус', offer.status), ('Тип', offer.offer_type), ('Название', offer.display_title or offer.title), ('Магазин', offer.merchant or '—'),
        ('ГЕО', _geo_label(offer)), ('Geo scope', offer.geo_scope), ('Город', offer.city or '—'), ('Регион', offer.region or '—'),
        ('Условия', offer.conditions or '—'), ('Макс. скидка', max_discount), ('Мин. сумма заказа', min_order),
        ('Категория', f'{offer.category or "—"} / {offer.subcategory or "—"}'), ('Скидка', str(offer.discount_percent or '—')),
        ('Промокод', offer.promo_code or '—'), ('Описание', offer.description or '—'), ('Первое появление', str(offer.first_seen_at)),
        ('Последнее появление', str(offer.last_seen_at)), ('Ссылка', link),
    ]
...
def runs_page(status: str = Query(''), source: str = Query('')):
    redirect = _require_setup()
    if redirect: return redirect
    status, source = status.strip(), source.strip()
    with create_session() as session:
        query = select(ParseRun, Source).outerjoin(Source, Source.id == ParseRun.source_id)
        if status: query = query.where(ParseRun.status == status)
        if source: query = query.where(Source.key == source)
    ...
    def opts(values, current, all_label):
        ...
        return ''.join(out)
    filters = f'''<div class="card"><form method="get" class="filters" style="grid-template-columns:1fr 1fr auto"><div class="field"><label>Статус</label><select name="status">{opts([(x,x) for x in ('running','success','partial','failed')],status,'Все')}</select></div><div class="field"><label>Источник</label><select name="source">{opts([(x.key,x.name) for x in sources],source,'Все')}</select></div><button class="btn">Применить</button></form></div>'''
    run_rows = []
    for run, src in rows:

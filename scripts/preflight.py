...
def free_local_port() -> int:
    """Reserve a temporary loopback port for isolated doctor validation."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="discount-parser-preflight-") as tmp:
        ...
        # A developer may already have another local application on the default
        # UI port. Preflight validates this project's port check in an isolated
        # environment instead of treating that external process as a defect.
